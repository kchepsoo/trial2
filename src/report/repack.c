...
/* Records collected in wire order, tagged with their source section. */
typedef struct dtl_repack_item {
    uint16_t   section_index;
    uint16_t   section_type;
    dtl_record rec; /* shallow copy; payload owned by the absorb arena */
} dtl_repack_item;

typedef struct dtl_repack_ctx {
    dtl_arena       *a;
    dtl_repack_item *items;
    size_t           count;
    size_t           cap;
} dtl_repack_ctx;

static dtl_err dtl_repack_on_record(const dtl_walk_event *ev, void *user)
{
    dtl_repack_ctx *ctx = user;
    dtl_repack_item *it;

    if (ctx->count == ctx->cap) {
        size_t new_cap = ctx->cap ? ctx->cap * 2 : 64;
        dtl_repack_item *grown =
                        ...
                        uint8_t codec_id, size_t max_file)
{
    ...
    if (dtl_codec_get(codec_id) == NULL)
        return DTL_ERR_INVAL;
    ...
    w.on_record = dtl_repack_on_record;
    w.on_error = NULL;
    w.user = &ctx;
    w.max_decode = 0;
    ...
    for (i = 0; i < ctx.count; i++) {
        const dtl_repack_item *it = &ctx.items[i];
