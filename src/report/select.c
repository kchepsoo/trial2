...
typedef struct dtl_select_item {
...
typedef struct dtl_select_ctx {
    dtl_arena        *a;
    const dtl_query  *q;
    dtl_select_item  *items;
    size_t            count;
    size_t            cap;
} dtl_select_ctx;

static dtl_err dtl_select_on_record(const dtl_walk_event *ev, void *user)
{
    dtl_select_ctx *ctx = user;
    dtl_select_item *it;

    if (!dtl_query_match(ctx->q, ev->rec))
        return DTL_OK;

    if (ctx->count == ctx->cap) {
        size_t new_cap = ctx->cap ? ctx->cap * 2 : 64;
        dtl_select_item *grown =
                        ...
                        const char *expr, size_t max_file, size_t *out_kept)
{
    ...
    if (in_path == NULL || out_path == NULL || expr == NULL)
    ...
    w.on_record = dtl_select_on_record;
    w.on_error = NULL;
    w.user = &ctx;
    w.max_decode = 0;
    ...
    for (i = 0; i < ctx.count; i++) {
        const dtl_select_item *it = &ctx.items[i];
